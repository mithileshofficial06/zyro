...
**Both paths are built.** `subgraph/` serves the schema with AssemblyScript
handlers; `substreams/` serves the *same* schema through `graph_out`
`EntityChanges`, so the Substreams package is a port rather than a second data
model. The Rust toolchain took some persuading on Windows — see
[`substreams/README.md`](../substreams/README.md) for the three
dependency-conflict decisions and the MinGW path-with-spaces trap.

That leaves **four implementations of the same Avellaneda–Stoikov arithmetic**:
Solidity, TypeScript, AssemblyScript, and (for the program decoding) Rust. Every
one is pinned to fixtures generated by a live `forge test` run, because a
published reservation price the chain would not quote is worse than none at
all — a solver routes on it and the execution disagrees.
